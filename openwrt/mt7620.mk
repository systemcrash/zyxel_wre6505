...
define Build/append-string-newline
	echo $(1) >> $@
endef

define Build/pad-with-ff-to
# there are more efficient ways than this - I was lazy...
# first pad to our desired block size into a tmp file
	dd if=$@ bs=$(1) conv=sync of=$@.tmp
# then get output size
	#make new file filled with FF of this size
	dd if=/dev/zero bs=$$(wc -c $@.tmp | awk '{ print $$1 }') count=1 | tr "\000" "\377" > $@.filled
	#write original into filled file
	dd if=$@ of=$@.filled conv=notrunc
	#clean up
	rm -f $@.tmp
	mv -f $@.filled $@
endef
...
#  KERNEL_SIZE := 1179648
  ...
  KERNEL := $(KERNEL_DTB) | uImage lzma | append-string-newline EdI0412zYx
#  zzzKERNEL := $(KERNEL_DTB) | uImage lzma | append-string-newline EdI0412zYx | pad-with-ff-to 64k
#  | pad-with-ff-to 64k
  DEVICE_PACKAGES := kmod-mt76x0e kmod-usb2 kmod-usb-ohci kmod-usb-ehci kmod-rt2800-soc kmod-leds-gpio \
 kmod-gpio-button-hotplug kmod-usb-ledtrig-usbport
  IMAGES += factory.bin
  IMAGE/factory.bin := $$(sysupgrade_bin) 
#| prependmagic EdI0412zYx
# pciutils: for debugging
# rt2800-pci-firmware: potentially unnecessary - devices have this on mtd partition
# kmod-leds-gpio: to assign the LEDs to the GPIO pins.
# kmod-gpio-button-hotplug: in case anyone wants to assign stuff to the buttons?
# kmod-usb-ledtrig-usbport: in case anyone wants to set up usb ports and triggers later on.
